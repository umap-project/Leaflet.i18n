...
// It differs from the original version in that allov to deep folder search
...
dirs.forEach(function (dir_path) {
    var findFilesRecursive = function (dirPath) {
        var files = fs.readdirSync(dirPath);
        files.forEach(function(file){
            var pathJoin = path.join(dirPath, file);
            var isDir = fs.lstatSync(pathJoin).isDirectory();
            if (isDir && file !== "node_modules") {
                findFilesRecursive(pathJoin);
            } else if (path.extname(file) === '.js')  {
                code += fs.readFileSync(pathJoin, "utf8");
            }
        });
    };
    findFilesRecursive(dir_path);
});
